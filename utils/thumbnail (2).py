...
async def load_profile_photo(client, user, size):
    try:
        ...
        pfp = Image.open(io.BytesIO(file.getvalue())).convert("RGBA")
...
async def generate_profile_card(client, user, stats):
    ...
    # ── Circle geometry (MATCH TEMPLATE) ──
    CIRCLE_SIZE = 320
    CIRCLE_X = 170
    CIRCLE_Y = 110

    # ── Load + crop real profile photo ──
    ...
    # ── Username (perfectly centered below circle) ──
    ...
    # ── Stats values (labels already on template) ──
    value_x = 750
    start_y = 105
    gap = 48

...
async def migrate():
    async with db.pool.acquire() as conn:
        ...
        INSERT INTO achievement_meta (key, value)
        ...
        """)
        # ───────────────── GAMES ─────────────────
        # Tracks individual match results and settings
        await conn.execute("""
        CREATE TABLE IF NOT EXISTS games (
            game_id UUID PRIMARY KEY,
            chat_id BIGINT,
            title TEXT, -- 🚀 Added to support "Already playing in [Group Title]"
            mode TEXT,
            host_id BIGINT,
            ...
            team_a_balls INT DEFAULT 0,
            team_b_balls INT DEFAULT 0,
            team_a_penalty INT DEFAULT 0,
            team_b_penalty INT DEFAULT 0,
            target INT,
        ...
        # ───────────────── TEAM SHIFTS ─────────────────
        # 🚀 Added to support get_shift_count and increment_shift helpers
        await conn.execute("""
        ...
        CREATE TABLE IF NOT EXISTS user_stats (
            ...
            penalties_received INT DEFAULT 0, -- 🚀 Added for lifetime penalty tracking
            created_at TIMESTAMP DEFAULT NOW()
        );
        """)
        ...
        # ───────────────── SAFETY COLUMN UPDATES ─────────────────
        # Updates for 'games' table
